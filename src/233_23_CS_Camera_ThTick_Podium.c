...
void CS_Camera_ThTick_Podium(struct Thread *th)
{
    ...
    if (   (   (   (OVR_233.cutsceneState != 0)
                || (*(u_int *)0x800b0b84 != 0)) // if pressed Start to skip scene
            && ((gGT->gameMode2 & 0x1000) != 0) // if cup is beaten now, that was not previously beaten
           )
        && (sdata->ptrActiveMenu == 0) // if XA is not playing
    )
    {
    ...
    )
    {
        ...
        if (iVar8 != 0)
        {
            ...
            if (iVar8 - 4800 < iVar9) OVR_233.PodiumInitUnk3 = 1;
    ...
    {
        if ((gGT->gameMode2 & 0x1000) != 0) goto LAB_800af2ec;
    ...
    )
    {
        ...
        if (rewardId != STATIC_BIG1)
        {
            ...
            if (CS_Camera_BoolGotoBoss() == 0)
            {
                ...
                if (   (rewardId != STATIC_GEM || GET_REWARD_COLOR(gGT->podiumRewardID) != 5)
                    && !skip_mask_congrats
                )
                {
                    switch (rewardId)
