...
void DECOMP_UI_CupStandings_InputAndDraw(void)
{
    ...
    if (gGT->numPlyrCurrGame != 1)
    {
        ...
        if(!RaceFlag_IsFullyOnScreen()) return;
    ...
    if (   sdata->framesSinceRaceEnded < 60
        && (sdata->AnyPlayerTap & 0x50) != 0 // If you press Cross or Circle
    )
    {
    ...
    sprintf(
        text,
        "%s %ld/4",
        sdata->lngStrings[0x175], // TRACK
        gGT->cup.trackIndex + 1 // Track Index (0, 1, 2, 3) + 1
    );
    ...
    {
        if (   (   (0x3b < sdata->framesSinceRaceEnded) // Timer
                && ((sdata->menuReadyToPass & 8) == 0) // If you are not in overall Cup standings
               )
    ...
    else
    {
        ...
        if (uVar8 == 0)
        {
            ...
            for (i = 0; i < numDrivers; i++)
            {
                ...
                gGT->cup.points[gGT->driversInRaceOrder[i]->driverID] += points[i];
            }
        ...
        else
        {
            ...
            {
                if ((gGT->gameMode2 & 0x10) != 0) UI_CupStandings_FinalizeCupRanks();
                ...
                else
                {
                    ...
                    if (   ((gGT->drivers[0]->driverRank) == 0)
                        ...
                        && ((gGT->gameMode1 & ARCADE_MODE) != 0) // If you're in Arcade Mode
                    )
                    {
