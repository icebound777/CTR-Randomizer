...
void AH_Door_ThTick(struct Thread* t)
{
    ...
    if (   (0x8ffff < dist) // if player is far from the door
        && ((door->camFlags & WdCam_CutscenePlaying) == 0)) // flags
    {
        return;
    }
                    ...
                    for (i = 0; i < numKeys; i++)
                    {
                        ...
                        if (keyInst != NULL)
                        {
                            ...
                            if (1 < numKeys)
                            {
                                ...
                                if (iVar17 < 0) iVar17 += 0x1f;

                                ratio = MATH_Sin(door->keyOrbit + iVar18);

                                keyInst->matrix.t[0] = (
                                    driver->instSelf->matrix.t[0]
                                    + ((iVar17 >> 5) * ratio >> 0xc)
                                );
    ...
    if (   (   (lev == N_SANITY_BEACH)  // if this is N Sane Beach
            && (doorID == 4)  // if this is door #4 (beach -> glacier)
           )
        || (lev == THE_LOST_RUINS) // if this is lost ruins (ruins -> glacier)
    )
    {
        // open all doors to glacier
        sdata->advProgress.rewards[3] |= 0xc0;
    }
    else if (   (lev == N_SANITY_BEACH) // if this is N Sane Beach
             && (doorID == 5) // Door #5 (beach -> ruins)
    )
    {
        // record that door is open
        sdata->advProgress.rewards[3] |= 0x10;
    }
    else if (lev == GEM_STONE_VALLEY) // Gemstone valley (cup door)
    {
        // record that door is open
        sdata->advProgress.rewards[3] |= 0x20;
    }
    else // Glacier Park (glacier -> citadel)
    {
        // record that door is open
