...
void DECOMP_UI_DrawRaceClock(u_short paramX, u_short paramY, u_int flags, struct Driver* driver)
{
    ...
    else
    {
        ...
        // this particular snippet is for drawing the TOTAL text in the time trial
        // lap results screen before it flashes, at which point it moves from the
        // left to the center of the screen
        // if global game timer is on an odd number
        // (in which case this condition doesn't execute)
        // then set string to end at posX of next DrawLine and use data.ptrColor[WHITE]
        if ((   (flags & 4) == 0)
             || (strFlags_but_its_also_posY = (JUSTIFY_RIGHT | WHITE), (gGT->timer & FPS_DOUBLE(2)) != 0)
        )
        {
    ...
    // str = 0xc5: YOUR TIME
    DecalFont_DrawLine(
        sdata->lngStrings[str],
        (int)(short) paramX,
        (int)(short) paramY,
        fontType,
        (int) strFlags_but_its_also_posY
    );

    // set string to use data.ptrColor[1], which is the periwinkle gradient seen
    // in the LAP text on the HUD
    ...
    (
        ...
        (
            ...
            // if the string (which is probably the total time text in the
            // time trial lap results screen) should flash
            (flags & 4) != 0
        )
    )
    {
    ...
    if (gGT->numLaps >= 7)
    {
        ...
        // Convert each number from the binary version of the number to the
        // ascii version of the number by adding ascii value of '0'
    ...
    // Convert each number from the binary version of the number to the ascii
    // version of the number by adding ascii value of '0'
    ...
    // If in-race, reuse X and Y positions used for the TIME/TIME TRIAL text in
    // the top left corner of the HUD for the actual time itself
    ...
    // If in the time trial results screen, reuse X and Y positions used for the
    // rightmost margin of the "TOTAL" text used for displaying the total time,
    ...
    DecalFont_DrawLine(displayTime, posX, numParamY >> 0x10, FONT_BIG, (int)strFlags_but_its_also_posY);
    #else
    DecalFont_DrawLine(
        totalTimeString,
        posX,
    ...
    if ((driver->driverID == 0) && (driver->lapIndex > 0))
    {
        ...
        DecalFont_DrawLine("BEST: ", posX, textPosY + 0x18, FONT_SMALL, RED);
        DecalFont_DrawLine(
            displayTime,
            posX + data.font_charPixWidth[FONT_SMALL] * 5,
            textPosY + 0x18,
            FONT_SMALL,
            PERIWINKLE
        );
        ...
        tt.minutes = min(tt.minutes, 9);
        sprintf(displayTime, "%d:%02d.%03d", tt.minutes, tt.seconds, tt.miliseconds);
        DecalFont_DrawLine("LAST: ", posX, textPosY + 0x18 + 8, FONT_SMALL, RED);
        DecalFont_DrawLine(
            displayTime,
            posX + data.font_charPixWidth[FONT_SMALL] * 5,
            textPosY + 0x18 + 8,
    ...
    )
    {
        ...
        do
        {
            if ((numLaps <= (int)lapIndex) && (numLaps < (char)gGT->numLaps))
            {
                ...
                // Convert each number from the binary version of the number to
                //the ascii version of the number by adding ascii value of '0'
                // This is dynamically programmed to handle time for more than
                // one player, see UI_SaveLapTime
                ...
                )
                {
                    ...
                    stringColor_but_its_also_relicColor = (
                        ((u_short)(gGT->timer >> FPS_DOUBLE(1)) ^ 1)
                        & 1
                    );
                }

                // Otherwise, color is white by default, you can see that in
                // "stringColor_but_its_also_relicColor = 1" near lap 3 check
                ...
                else
                {
                    ...
                    DecalFont_DrawLine(
                        local_38,
                        unbitshiftTextPosX,
                        (int)(((u_int)textPosY - ((char)gGT->numLaps - numLaps) * (int)*lapTextHeight) * 0x10000) >> 0x10,
                        lapFontType,
                        (JUSTIFY_RIGHT | RED)
                    );
                    ...
                    // LAP
                    DecalFont_DrawLine(
                        sdata->lngStrings[0x18],
                        (int)(((u_int)textPosX - (u_int)data.font_charPixWidth[lapFontType])),
                        (int)(((u_int)textPosY - ((char)gGT->numLaps - numLaps) * (int)*lapTextHeight) * 0x10000) >> 0x10,
                        lapFontType,
                        (JUSTIFY_RIGHT | RED)
                    );
...
LAB_8004f84c:
    ...
    DecalFont_DrawLine(
        ...
        (int)sVar1,
        fontType,
        (int)(short)stringColor_but_its_also_relicColor
    );
