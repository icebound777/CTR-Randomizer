...
void DECOMP_AH_Pause_Draw(int pageID, int posX)
{
    ...
    if(type == 0)
    {
        ...
        if(hubID == 0)
        {
            ...
            for(int i = 0; i < 2; i++)
            {
                ...
                inst->matrix.t[0] =
                    UI_ConvertX_2(posX + 0x16d + 1*0x1e, 0x100);

                inst->matrix.t[1] =
                    UI_ConvertY_2(i*0x10 + 4 + 0x2f, 0x100);
        ...
        else
        {
            ...
            for(int i = 0; i < 4; i++)
            {
                ...
                for(int j = 0; j < 3; j++)
                {
                    ...
                    // Remove SelectProfile with regular UI variant
                    inst->matrix.t[0] =
                        UI_ConvertX_2(posX + 0x15e + j*0x1e, 0x100);

                    inst->matrix.t[1] =
                        UI_ConvertY_2(i*0x10 + 0 + 0x2f, 0x100);
                }
            ...
            // Remove SelectProfile with regular UI variant
            inst->matrix.t[0] =
                UI_ConvertX_2(posX + 0x15e + 1*0x1e, 0x100);

            inst->matrix.t[1] =
                UI_ConvertY_2(4*0x10 + 0 + 0x2f, 0x100);
            ...
            inst->matrix.t[1] =
                UI_ConvertY_2(5*0x10 + 0 + 0x2f, 0x100);
    ...
    {
        char* ptrTokenCount = (char*) &((advSlot2->rewards[2]));
        ...
        for(int i = 0; i < 5; i++)
        {
            ...
            // Remove SelectProfile with regular UI variant
            inst->matrix.t[0] =
                UI_ConvertX_2(instPosX, 0x100);

            inst->matrix.t[1] =
                UI_ConvertY_2(instPosY + 0x41, 0x100);
    ...
    RECTMENU_DrawOuterRect_Edge(&r, sdata->battleSetup_Color_UI_1, 0x20, ot);
