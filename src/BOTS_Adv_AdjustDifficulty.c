...
void DECOMP_BOTS_Adv_AdjustDifficulty(void)
{
    ...
    else // If you're not in a Boss Race
    {
    ...
    if ((gameMode1 & ARCADE_MODE) == 0)
    {
        ...
        if ((gameMode1 & ADVENTURE_CUP) == 0)
        {
            ...
            if ((int)gameMode1 < 0)
            {
                ...
                baseModifier = BOTS_Adv_NumTimesLostEvent(
            ...
            {
                /* START Randomizer */
                struct AdvProgress *advSlot2 = (
                    (struct AdvProgress*) (sdata->memcardBytes + 0x50 + 4)
                );
                struct AdvProgress *advSlot3 = (
                    (struct AdvProgress*) (sdata->memcardBytes + 0xA0 + 4)
                );
                /* END Randomizer */
                short trophyIndex = (
                    advSlot2->SLOT2_NUM_TROPHIES
                    + advSlot3->SLOT2_NUM_TROPHIES
                ) + 1;
                baseModifier = BOTS_Adv_NumTimesLostEvent(
                    sdata->advProgress.timesLostRacePerLev[gGT->levelID]
                );
        ...
        else // If you're in Adventure Cup
        {
            ...
            baseModifier = BOTS_Adv_NumTimesLostEvent(
                sdata->advProgress.timesLostCupRace[track]
            );
            ...
            else // If you're in Adventure Cup (Red, Green, Blue, Yellow)
            {
    ...
    if ((currDifficulty << 0x10) < 0) currDifficulty = 0;

    for (i = 0; i < 14; i++)
    {
        difficultyParams[i] = (
            params1[i]
            + (short) ((currDifficulty * (params2[i] - params1[i])) / 0xf0)
        );
    }
    ...
    )
    {
        for (i = 0; i < 14; i++)
        {
            difficultyParams[i] = (
                params1[i]
                + (short) ((cupDifficulty * (params2[i] - params1[i])) / 0xf0)
            );
        }
    }
    ...
    if (   (sdata->const_0x30215400 == 0)
        && (sdata->const_0x493583fe == 0)
    )
    {
    ...
    )
    {
        ...
        if ((gameMode1 & (LOADING | TIME_TRIAL | RELIC_RACE)) == 0)
        {
            ...
            if ((gameMode1 & CRYSTAL_CHALLENGE) == 0)
            {
                ...
                else // If you're not in Boss Race
                {
                    if (   ((gameMode1 & ADVENTURE_CUP) == 0) // If you're not in Adventure Cup
                        || (gGT->cup.cupID != 4) // If this is not the purple gem cup
                    )
                    {
                        if ((gameMode1 & ADVENTURE_MODE) == 0) // If you're not in Adventure Mode
                        {
                            // {0, 1, 2, 3, 4, 5, 6, 7}
                    ...
                    else // If this is Purple Gem Cup
                    {
                        // {0, 1, 2, 3, 4, 5, 6, 7}
            ...
            else // If you're in Crystal Challenge (in adventure mode)
            {
                // {0, 1, 2, 3, 4, 5, 6, 7}
        ...
        else // If you're Loading or in Time Trial
        {
            // {1, 2, 0, 3, 4, 5, 6, 7}
    ...
    )
    {
        ...
        for (i = 0; i < 8; i++)
        {
            if (   (gGT->numPlyrCurrGame <= i)
                && (playerPoint < gGT->cup.points[i]) // point has not been updated?
            )
            {
